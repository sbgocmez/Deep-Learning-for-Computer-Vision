...
def mutate_tensor(x, indices, values):
  ...
  for index, element in enumerate(indices):
    #print(" element: ", element, "  index no: ", index)
    x[element] = values[index] 
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def multiples_of_ten(start, stop):
  ...
  # Replace "pass" statement with your code
  # given 6 and 32
  # start -> 6 % 10 = 6, start += 10-6 -> start = 6+4 = 10
  # stop -> 32%10 = 2, stop -= 2 = 32 - 2 = 30
  # given 5 and 7
  # start -> 5 += 10 - 5 = 5+5 = 10
  # stop -> 7-7 = 0
  # now, stop < start. also, when stop is given as 13,
  # 13 -> 13 - 3 = 10. stop == start.
...
def count_negative_entries(x):
  ...
  # Replace "pass" statement with your code
  mask = (x < 0)
  new_tensor = torch.clone(x[mask])
  num_neg = count_tensor_elements(new_tensor)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_neg
...
def zero_row_min(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  row_min_idxs = torch.argmin(x, dim=1, keepdim=True)
  y = x.clone()
  y[torch.arange(y.size(0)).unsqueeze(1), row_min_idxs] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone()
  mean = torch.sum(y, dim=0)/y.shape[1]
  v = mean.repeat((y.shape[1],1))
  w = (y-v)*(y-v)
  std = torch.sqrt(torch.sum(w, dim=0)/(y.shape[1]-1))
  ...
  print("V IS: ", (x-v)*(x-v))
  w = (x-v)*(x-v)
  print("original v becomes: ", torch.sqrt(torch.sum(w, dim=0)/(x.shape[1]-1)))
  print("STD : ", torch.sqrt((x-v)*(x-v)/(x.shape[1]-1)))
  print(torch.sum(x-v, dim=0)*torch.sum(x-v, dim=0))
  
  std = torch.sum(x-mean, dim=0)*torch.sum(x-mean, dim=0)/(x.shape[1]-1)**2

  print("my std: ", std)
  print("std: ", torch.std(x, dim=0))
  print(x.shape[1])
  print(torch.mean(x, dim=0))
  print("multiply by 3 is: ",x)
  print("SUM : ", torch.sum(x, dim=0)/x.shape[1])
  v = x.repeat((x.shape),1)
  print("v is : ", v)
  #print("x+v is: ", x+v)
  print("MEAN OF COLUMN: " , torch.mean(x, dim=0))
